...
function zoneBestRateMovies() {
  fetch("http://localhost:8000/api/v1/titles/?actor=&actor_contains=&company=&company_contains=&country=&country_contains=&director=&director_contains=&format=json&genre=&genre_contains=&imdb_score=&imdb_score_max=&imdb_score_min=&lang=&lang_contains=&max_year=&min_year=&rating=&rating_contains=&sort_by=-imdb_score&title=&title_contains=&writer=&writer_contains=&year=")
     ...
     .then((data) => {
       const json = data;
  let zoneBestRateMovies = document.getElementById('zoneBestRateMovies');
  ...
  for(let index=0; index<=NUMBER_OF_IMAGES; index++) {
    ...
    zoneBestRateMovies.appendChild(div);
...
function zoneBestRateMovies_2() {
  fetch("http://localhost:8000/api/v1/titles/?actor=&actor_contains=&company=&company_contains=&country=&country_contains=&director=&director_contains=&format=json&genre=&genre_contains=&imdb_score=&imdb_score_max=&imdb_score_min=&lang=&lang_contains=&max_year=&min_year=&page=2&rating=&rating_contains=&sort_by=-imdb_score&title=&title_contains=&writer=&writer_contains=&year=")
     ...
     .then((data) => {
       const json = data;
  let zoneBestRateMovies_2 = document.getElementById('zoneBestRateMovies_2');
  ...
  for(let index=0; index<=NUMBER_OF_IMAGES; index++) {
    ...
    zoneBestRateMovies_2.appendChild(div);

  }
})

};
zoneBestRateMovies_2()
