...
function category_1() {
   fetch("http://localhost:8000/api/v1/titles/?actor=&actor_contains=&company=&company_contains=&country=&country_contains=&director=&director_contains=&format=json&genre=Action&genre_contains=&imdb_score=&imdb_score_max=&imdb_score_min=&lang=&lang_contains=&max_year=&min_year=&rating=&rating_contains=&sort_by=-imdb_score&title=&title_contains=&writer=&writer_contains=&year=")
    ...
    for(let index=0; index<=NUMBER_OF_IMAGES; index++) {
        ...
        let a = createA();
        a.className = "button";
        a.setAttribute("href","#popup"+index+10);

        a.appendChild(createImgPopup(affiche, title));
        div.appendChild(a);
        td.appendChild(div);

        category_1.appendChild(td);
...
function category_1_Part2() {
   fetch("http://localhost:8000/api/v1/titles/?actor=&actor_contains=&company=&company_contains=&country=&country_contains=&director=&director_contains=&format=json&genre=Action&genre_contains=&imdb_score=&imdb_score_max=&imdb_score_min=&lang=&lang_contains=&max_year=&min_year=&page=2&rating=&rating_contains=&sort_by=-imdb_score&title=&title_contains=&writer=&writer_contains=&year=")
    ...
    let category_1_Part2 = document.getElementById('category_1_Part2');
    const NUMBER_OF_IMAGES = 2;
    for(let index=0; index<=NUMBER_OF_IMAGES; index++) {
        let td = createTd();
        let affiche = cleanString(json.results[index].image_url);
        ...
        let a = createA();
        a.className = "button";
        a.setAttribute("href","#popup"+index+14);

        a.appendChild(createImgPopup(affiche, title));
        div.appendChild(a);
        td.appendChild(div);

        category_1_Part2.appendChild(td);
